...
class Request():
    ...
    @classmethod
    def config(cls, num_pools: int = 10, header: str = None, **kwargs) -> None:
        cls._req = urllib3.PoolManager(num_pools, header, **kwargs)

    @classmethod
    def get(
            cls, url: str, header: str = None, is_json_response: bool = True
        ...
        try:
            ...
            logger.opt(exception=True).error(
                'Max http connection pool is reached')
        except TimeoutError:
            logger.opt(exception=True).error(
                'Timeout in http request to url:{}', url)
        except Exception:
            logger.opt(exception=True).error(
    ...
    @classmethod
    def post(
            cls, url: str, fields: Dict = None,
            header: str = None, is_json_response: bool = True,
            ) -> Optional[ResponseModel]:
